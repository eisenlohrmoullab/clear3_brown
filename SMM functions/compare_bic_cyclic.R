...
compare_bic_cyclic <- function(
  ...
  time_var <- ifelse(centering == "menses", "cyclic_time_impute", "cyclic_time_imp_ov")
  ...
  # Decide which variable to model (log-transformed or not)
  outcome_var <- if (log_var) paste0(outcome, "_log.d") else paste0(outcome, ".d")
  
  # Fit single-group model
  formula_g1 <- as.formula(glue("{outcome_var} ~ s({time_var}, bs = 'cc', k = {k_smooth})"))
  
  gamm_fit_g1 <- gamm4::gamm4(
    formula_g1,
