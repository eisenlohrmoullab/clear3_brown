...
run_smm_cyclic <- function(
  ...
  ## --------- create nested folder structure: date / outcome / centering ----------
  if (!is.null(save_dir)) {
    date_folder <- format(Sys.Date(), "%Y%m%d")
    centering_folder <- if (centering == "ovulation") "ovulation_centered" else "menses_centered"
    
    # Final path: save_dir/YYYYMMDD/outcome/centering_folder
    sub_dir <- file.path(save_dir, date_folder, outcome, centering_folder)
  ...
  if (length(g) > 1) {
    ...
    for (gval in g) {
      ...
      res <- run_smm_cyclic(
        data = data, outcome = outcome, g = gval, d_inter = d_inter,
        plot = plot, seed = seed, centering = centering, save_dir = save_dir,
        k_smooth = k_smooth, log_var = log_var
      )
  ...
  ## Choose cyclic time variable
  time_var <- if (centering == "ovulation") "cyclic_time_imp_ov" else "cyclic_time_impute"
  if (!time_var %in% names(data)) {
    stop(glue("Time variable {time_var} not found in data!"))
  }
  
  ## Subset columns
  outcome_cols <- c(outcome, paste0(outcome, ".d"), paste0(outcome, ".d.roll"), paste0(outcome, "_log.d"))
  ...
  if (plot) {
    ...
    summary_roll <- df %>%
      ...
      dplyr::summarise(
        ...
        se = sd(.data[[paste0(outcome, ".d.roll")]], na.rm = TRUE) /
          sqrt(sum(!is.na(.data[[paste0(outcome, ".d.roll")]]))),
        .groups = "drop"
      ) 
    ...
    summary_centered <- df %>%
      ...
      dplyr::summarise(
        ...
        se = sd(.data[[paste0(outcome, ".d")]], na.rm = TRUE) /
          sqrt(sum(!is.na(.data[[paste0(outcome, ".d")]]))),
        .groups = "drop"
      ) 
