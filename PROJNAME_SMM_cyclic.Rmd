---
title: "SMM Analysis Pipeline - Production Version"
output: html_document
---

# 🚀 SMM Analysis Pipeline 

> 👉 **START HERE** - This is the main analysis script for production use.
> 
> 📚 **First time?** See [`docs/examples/QUICK_START.md`](docs/examples/QUICK_START.md) for a detailed guide.
> 
> 🧪 **Want to test first?** Use [`TEST_SMM_cyclic.Rmd`](TEST_SMM_cyclic.Rmd) with example configurations.

---

## Description

This script performs **Smooth Mixture Modeling (SMM)** analysis on cyclic hormone and behavioral data across the menstrual cycle.

**Pipeline Features:**
- ✅ Multiple centering methods (menses or ovulation)
- ✅ **Per-outcome configuration:** Each variable can have its own winsorization and rolling average settings
- ✅ **Flexible data preprocessing:** Choose none, 3-day, or 5-day rolling averages per outcome
- ✅ Multiple outcome variables analyzed simultaneously
- ✅ Automatic model comparison with BIC
- ✅ Publication-ready GAM visualizations
- ✅ Automated group probability comparisons within each configuration
- ✅ Comprehensive logging and error handling
- ✅ Optional parallel processing for speed

**Requirements:**
- Data frame named `cycle_df_scaled` loaded in your R environment
- Required columns: `id`, time variables (menses & ovulation), outcome variables
- All helper scripts in the same directory (auto-loaded)

**Quick Usage:**
1. Load your data: `cycle_df_scaled <- read.csv("your_data.csv")`
2. Configure parameters in Section 2 below:
   - Set project name and paths
   - **Configure each outcome** with winsorization and rolling average settings
   - Choose centering methods to compare
3. Run entire script: Click "Run All" in RStudio or knit to HTML

**Outputs:**
- Analysis results: `base_save_dir/project_name/smm/YYYYMMDD/outcome/winsorized|unwinsorized/rollXXX/centering/`
- Group probability comparisons: `base_save_dir/project_name/smm/YYYYMMDD/group_probability_comparisons/`
- Log file: `./logs/project_name_smm_analysis_YYYYMMDD_HHMMSS.log`

For more information, see: `README_SMM_ENHANCED.md`
...
```{r user-params}
project_name <- "TESTADHD24_5daytasks"  # Change this to your project's short name
...
outcomes_config <- data.frame(
  outcome = c("E2", "P4", "LH", "CSS_Inatt", "CSS_HypImp", "score_robot_rev", "score_pinball_rev", "DRSP_1", "DRSP_4", "DRSP7", "DRSP_22"),
  winsorized = c(TRUE, TRUE, TRUE, FALSE, FALSE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE),  # TRUE = apply winsorization
  rolling_avg = c("3day", "3day", "3day", "3day", "3day", "5day", "5day", "3day", "3day", "3day", "3day"),  
...
```{r parallel}
use_parallel <- TRUE  # Set to TRUE to enable parallelization
...
```{r winsorize-fn}
winsorize_by_person <- function(data, outcome, percentile = 0.98) {
...
**IMPORTANT:** These comparison scripts were designed for the old approach where each outcome was analyzed with all 4 combinations (2 centering × 2 winsorization). With the new per-outcome configuration, you may see fewer comparisons or need to manually inspect the outputs. Future versions will update these scripts to better handle the new approach.
...
if (use_selected_configs) {
  ...
  if (valid_selections) {
    log_success("✓ All selected configurations found in results!")
  } else {
    log_warn("⚠ Some configurations were not found. Check your selections.")
...
- ✓ Results are immediately interpretable ("Luteal-Peak" vs "Group 2")
- ✓ Comparison tables show meaningful pattern names
- ✓ Cross-outcome patterns are easier to understand
- ✓ Better for presentations and publications
- ✓ Reproducible - labels are saved to CSV files
...
# =============================================================================
# This will prompt you to label yeach group for each outcome.
...
if (use_selected_configs && !is.null(outcome_config_selections)) {
  ...
  log_success("✓ Labeling complete for all selected configurations!")
...
└── E2/
    └── winsorized/
        └── menses_centered/
            ├── E2_g3_class.csv
            ├── E2_g3_group_labels.csv    ← Labels saved here
            └── E2_g3_centered.png
```
...
**📖 Additional resources:** For more examples and best practices, see `docs/examples/GROUP_LABELING_GUIDE.md`
...
for (i in 1:nrow(unique_configs)) {
  ...
  if (use_selected_configs && !is.null(outcome_config_selections)) {
    ...
    if (length(config_outcomes) == 0) {
      log_info(paste0("  ⊘ No outcomes selected for ", config_label, ". Skipping."))
      next
    }
    
    log_info(paste0("  ✓ Including ", length(config_outcomes), " outcome(s): ", 
    ...
    log_info(paste0("  ✓ Including all ", length(config_outcomes), " outcomes"))
  }
  ...
  if (comparison_results$status == "success") {
    log_success(paste0("✓ Completed comparisons for ", config_label))
    log_info(paste0("  Results: ", comparison_results$report_dir))
  } else {
    log_warn(paste0("⚠ Comparisons for ", config_label, " ", 
